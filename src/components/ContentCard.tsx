...
import { PlayCircle, BookOpen, CalendarDays, Share2 } from "lucide-react";
    ...
    <Card className="w-full h-full flex flex-col overflow-hidden rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 bg-neutral-900 text-white border-neutral-800">
      ...
      <CardHeader className="p-4 pb-2 flex-grow">
        <CardTitle className="text-xl font-bold leading-tight line-clamp-2">
          <Link to={link} className="hover:text-red-500 transition-colors">
            {title}
          </Link>
        </CardTitle>
      ...
      <CardFooter className="p-4 pt-0 flex items-center justify-between">
        <Link to={link} className="inline-flex items-center text-red-500 hover:text-red-400 text-sm font-semibold transition-colors">
          {getIcon()}
          Read More
        </Link>
        <Popover>
